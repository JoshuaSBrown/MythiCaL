...
void KMC_Particle::setMemoryCapacity(unsigned int numberSitesToRemember) {
  ...
  if (numberSitesToRemember == 1) {
    string err =
        "It does not make sense to have a memory of 1 because a "
  ...
  if (delta > 0) {
    for (int i = 0; i < delta; ++i) {
      Memory memory = {constants::unassignedId, constants::unassignedId, 0};
...
int KMC_Particle::getVisitationFrequencyOfCurrentlyOccupiedSite() {
  ...
  string err =
      "ERROR you cannot get the visitation frequency of the "
      "unoccupied site at this moment, as no sites have yet been occupied.\n";
  throw runtime_error(err);
}

void KMC_Particle::occupySite(const int siteId, const int clusterId) {
...
void KMC_Particle::setClusterSiteBelongsTo(const int siteId,
                                           const int clusterId) {
...
vector<vector<int>> KMC_Particle::getMemory() {
  ...
  for (auto memory : memoryQueue_) {
    if (memory.siteId != constants::unassignedId) {
      memories.push_back(
          vector<int>{memory.siteId, memory.clusterId, memory.visitFrequency});
    }
  }
  return memories;
}
...
list<KMC_Particle::Memory>::iterator KMC_Particle::getMemoryIteratorSite_(
    const int siteId) {
...
list<KMC_Particle::Memory>::iterator KMC_Particle::getMemoryIteratorCluster_(
    const int clusterId) {
